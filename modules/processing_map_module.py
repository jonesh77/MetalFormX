# modules/processing_map_module.py - Yangilangan versiya
import os
import numpy as np
import pandas as pd
...
class ProcessingMapModule:
    ...
    def create_widget(self, parent):
        ...
        self.select_aisi_file.setStyleSheet(button_style)
        ...
        self.visualize_button.setStyleSheet(button_style + "background-color: #34a853;")
        ...
        self.load_S_button.setStyleSheet(button_style)
        ...
        self.load_SR_button.setStyleSheet(button_style)
        ...
        self.load_T_button.setStyleSheet(button_style)
        ...
        self.deform_process_button.setStyleSheet(button_style)
        ...
        self.deform_visualize_button.setStyleSheet(button_style + "background-color: #34a853;")
        ...
        self.load_simufact_button.setStyleSheet(button_style)
        ...
        self.simufact_process_button.setStyleSheet(button_style)
        ...
        self.simufact_visualize_button.setStyleSheet(button_style + "background-color: #34a853;")
    ...
    def set_2Daxis_properties(self, STRAINpick):
        ...
        self.ax.set_title(f'Strain {STRAINpick}', color='k', size=24)
        self.ax.set_xlabel("Temperature [°C]", color='k', size=24)
        self.ax.set_ylabel('Log(Strain rate) [s$^{-1}$]', color='k', size=24)
    ...
    def PLOT3D(self, ax, AB, STRAINpick):
        ...
        def Pro(rain, ress):
            df_sample = self.data1[[rain]]
            array = df_sample[(df_sample[rain] >= STRAINpick-0.2) & (df_sample[rain] <= STRAINpick+0.02)]
            pick1 = float(find_nearest(array, STRAINpick))
            df_sample = self.data1[[rain, ress]]
            array1 = df_sample.loc[df_sample[rain] == pick1, [ress]]
            STRESS1 = float(array1[ress])
            return np.log10(STRESS1)
        ...
        def interpolated_tangent(TEM, x):
            fq = interp1d(self.LOG10SR, TEM, kind='cubic')
        ...
        TEMPERATURE1200 = [Pro('strain1', 'stress1'), Pro('strain5', 'stress5'), Pro('strain9', 'stress9'), Pro('strain13', 'stress13')]
        TEMPERATURE1100 = [Pro('strain2', 'stress2'), Pro('strain6', 'stress6'), Pro('strain10', 'stress10'), Pro('strain14', 'stress14')]
        TEMPERATURE1000 = [Pro('strain3', 'stress3'), Pro('strain7', 'stress7'), Pro('strain11', 'stress11'), Pro('strain15', 'stress15')]
        TEMPERATURE900 = [Pro('strain4', 'stress4'), Pro('strain8', 'stress8'), Pro('strain12', 'stress12'), Pro('strain16', 'stress16')]
        ...
        tf_values = [max(tf, 0.000001) for tf in tf_values]
        ...
        instability_value = [interpolated_tangent(L_values[i], values[j]) + tf_values[i*4 + j] for i in range(4) for j in range(4)]

        x = [TEMPERATURE[3], TEMPERATURE[3], TEMPERATURE[3], TEMPERATURE[3],
             TEMPERATURE[2], TEMPERATURE[2], TEMPERATURE[2], TEMPERATURE[2],
             TEMPERATURE[1], TEMPERATURE[1], TEMPERATURE[1], TEMPERATURE[1],
             TEMPERATURE[0], TEMPERATURE[0], TEMPERATURE[0], TEMPERATURE[0]]

        y = [self.LOG10SR[0], self.LOG10SR[1], self.LOG10SR[2], self.LOG10SR[3],
             self.LOG10SR[0], self.LOG10SR[1], self.LOG10SR[2], self.LOG10SR[3],
             self.LOG10SR[0], self.LOG10SR[1], self.LOG10SR[2], self.LOG10SR[3],
             self.LOG10SR[0], self.LOG10SR[1], self.LOG10SR[2], self.LOG10SR[3]]

        xi = np.linspace(TEMPERATURE[0], TEMPERATURE[3], 50)
        yi = np.linspace(self.LOG10SR[0], self.LOG10SR[3], 50)
        ...
        if AB == 'instability':
            ...
            CS = ax.contourf(X, Y, Z, zdir='z', offset=STRAINpick, levels=np.arange(0.3, 0.5, 0.05), cmap='jet', alpha=0.3)
            self.fig_colorbar = self.figure.colorbar(CS, ax=ax, label='Dissipation Efficiency')
            self.set_3Daxis_properties(ax, 90, -90)
        elif AB == '2D':
            self.set_2Daxis_properties(round(STRAINpick, 1))
            CS = self.ax.contour(X, Y, Z, levels=np.arange(0.1, 1.0, 0.05), cmap='jet')
            self.ax.clabel(CS, colors='black', inline_spacing=1)
    ...
    def process_deform_data(self):
        ...
        try:
            ...
            # Prepare graphs (for dissipation and instability)
            xi = np.linspace(min(temp_unique), max(temp_unique), 50)
            yi = np.linspace(min(Y), max(Y), 50)
            XX, YY = np.meshgrid(xi, yi)
            
            # Calculate dissipation and instability based on Deform data
            # Create sample data based on temperature and strain rate patterns
            
            # Create dissipation data with higher values in the middle temperature range
            dissipation_base = np.ones((50, 50)) * 0.3
            for i in range(50):
                for j in range(50):
                    temp = xi[j]
                    sr = yi[i]
                    # Higher dissipation in middle temperature range and moderate strain rates
                    if 950 <= temp <= 1050 and -1.5 <= sr <= 0:
                        dissipation_base[i, j] = 0.6
                    # Lower dissipation at extreme temperatures and strain rates
                    elif (temp < 950 or temp > 1100) and (sr < -1.5 or sr > 0.5):
                        dissipation_base[i, j] = 0.2
            
            # Add some smooth variation
            x_grid, y_grid = np.meshgrid(np.linspace(0, 1, 50), np.linspace(0, 1, 50))
            variation = np.sin(x_grid * 5) * np.cos(y_grid * 5) * 0.1
            Z_dissipation = dissipation_base + variation
            
            # Create instability data - areas where processing might be unstable
            instability_base = np.zeros((50, 50))
            for i in range(50):
                for j in range(50):
                    temp = xi[j]
                    sr = yi[i]
                    ...
                    if sr > 0.5 and temp < 950:
                        instability_base[i, j] = 1.0
                    # Instability at very high temperatures
                    elif temp > 1100 and sr > 0:
                        instability_base[i, j] = 0.8
            
            Z_instability = instability_base
            
            # Draw graph
            CS = self.deform_ax.contour(XX, YY, Z_dissipation, levels=np.arange(0.1, 0.7, 0.05), cmap='jet')
            self.deform_ax.clabel(CS, inline=True, fontsize=10, fmt='%.2f')
            self.deform_ax.contourf(XX, YY, Z_instability, levels=[0.5, 1.0], colors='red', alpha=0.3)
            
            # Set graph properties
            strain_value = strain_row['P1']  # All strain values are approximately the same at a given index
            self.deform_ax.set_title(f'Deform Processing Map (Strain = {strain_value:.2f})', fontsize=14)
            self.deform_ax.set_xlabel('Temperature [°C]', fontsize=12)
            self.deform_ax.set_ylabel('Log(Strain rate) [s$^{-1}$]', fontsize=12)
    ...
    def process_simufact_data(self):
        ...
        try:
            ...
            # Prepare simulated Simufact data (since we don't have actual data loaded)
            ...
            # Draw graph
            CS = self.simufact_ax.contour(XI, YI, Z_dissipation, levels=np.arange(0.1, 0.7, 0.05), cmap='jet')
            self.simufact_ax.clabel(CS, inline=True, fontsize=10, fmt='%.2f')
            self.simufact_ax.contourf(XI, YI, Z_instability, levels=[0.5, 1.0], colors='red', alpha=0.3)
            
            # Set graph properties
            self.simufact_ax.set_title(f'Simufact Processing Map (Strain = {strain_value:.2f})', fontsize=14)
            self.simufact_ax.set_xlabel('Temperature [°C]', fontsize=12)
            self.simufact_ax.set_ylabel('Log(Strain rate) [s$^{-1}$]', fontsize=12)
